            ...
            {isLoading || !p ? (
                ...
                <div className="mt-2 space-y-5 text-sm">
                    ...
                    <div className="grid grid-cols-2 gap-3 rounded-xl bg-slate-50 p-4">
                        ...
                        <div>
                            <p className="text-xs text-slate-400 uppercase tracking-wide">Cutoff</p>
                            <span className={`inline-block mt-0.5 rounded-full px-2.5 py-0.5 text-xs font-semibold ${p.cutoff_type === 'second' ? 'bg-violet-50 text-violet-600 border border-violet-200' : 'bg-sky-50 text-sky-600 border border-sky-200'}`}>
                                {p.cutoff_type === 'second' ? '2nd Cutoff · Govt deductions included' : '1st Cutoff · Basic pay only'}
                            </span>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        {p.cutoff_type === 'first' && (
                            <p className="mb-2 rounded-lg bg-sky-50 border border-sky-100 px-3 py-2 text-xs text-sky-700">
                                Government contributions (SSS, PhilHealth, Pag-IBIG, Withholding Tax) are deducted on the 2nd cutoff payslip.
                            </p>
                        )}
                        <div className="divide-y divide-slate-50">
                            {deductions.map((l) => (
